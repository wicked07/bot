...
(function() {
    ...
    settings.prototype.songIntervalMessages = [
      {
        interval: 15,
        offset: 0,
        msg: "Bot editado por Cafeinado - Weryques S.S."
      }
    ];
      ...
      if (timeSinceLastActivity > data.afkTime) {
        if (user.getIsDj()) {
          ...
          if (user.getWarningCount() === 0) {
            user.warn();
            _results.push(API.sendChat("@" + user.getUser().username + ", Eu não vi você mostrar atividade no chat durante 30 min.  Mostre atividade nos proximos 2 min., se não sera removido."));
            ...
            if (timeSinceLastWarning > twoMinutes) {
              ...
              warnMsg += ", Eu não vi você mostrar atividade durante 32 min. Mostre atividade no proximo minuto ou eu o removerei, este é seu ultimo aviso.";
            ...
            if (timeSinceLastWarning > oneMinute) {
              ...
              if (DJs.length > 0 && DJs[0].id !== user.getUser().id) {
                API.sendChat("@" + user.getUser().username + ", Você teve 2 avisos. Por favor, mantenha-se ativo no chat e votando.");
  ...
  newSongsCommand = (function(_super) {
    ...
    newSongsCommand.prototype.init = function() {
      this.command = '!newsongs(NÃO USAR)';
      this.parseType = 'startsWith';
  ...
  themeCommand = (function(_super) {
    ...
    themeCommand.prototype.functionality = function() {
      var msg;
      msg = "Todo tipo de musica eletronica de boa qualidade é permitida aqui. Incluindo Electro, dubstep, techno, trap, EDM, ";
      msg += "Hardstyle, House and Trance!!";
      return API.sendChat(msg);
    };
  ...
  rulesCommand = (function(_super) {
    ...
    rulesCommand.prototype.functionality = function() {
      var msg1, msg2;
      msg1 = "1) Tempo máximo 6 minutos e 30 segundos";
      msg1 += "2) Não escreva em /me ou /em  3) Respeite os moderadores da sala. ";
      msg1 = "4) Sem flood no chat  5) Não fique pedindo cargos "; 
	  msg1 += "6) Não fique sem votar nas musicas ";
	  msg2 = "1) Maximum time 6 minutes and 30 seconds";
	  msg2 += "2) Do not write to /me or /em  3) Respect the moderators of the room";
	  msg2 = "4) No flood the chat  5) Do not be asking positions";  
	  msg2 += "6) Don't be without vote in music"; 
      return API.sendChat(msg);
    };
  ...
  roomHelpCommand = (function(_super) {
    ...
    roomHelpCommand.prototype.functionality = function() {
      var msg1, msg2;
      msg1 = "Bem vindo a EDT! Crie uma playlist com musicas do youtube ou soundcloud.  ";
      msg1 += "Clique em 'Unir-se a lista de espera' e espere a sua vez de tocar.  ";
      msg2 = "Mantenha-se ativo enquanto espera para tocar sua musica ou será removido, não toque musicas repetidas (verifique em 'HISTORIA')  ";
      msg2 += "Evite tocar musicas de artistas como Skrillex (são muito tocadas), assim você diminui a probabilidade de ser pulado  ";
      API.sendChat(msg1);
  ...
  sourceCommand = (function(_super) {
    ...
    sourceCommand.prototype.init = function() {
      this.command = ['/source(NÃO USAR)', '/sourcecode(NÃO USAR)', '/author(NÃO USAR)'];
  ...
  downloadCommand = (function(_super) {
    ...
    downloadCommand.prototype.init = function() {
      this.command = '!download(NÃO USAR)';
      this.parseType = 'exact';
      return this.rankPrivelege = 'user';
    };
  ...
  statusCommand = (function(_super) {
    ...
    statusCommand.prototype.functionality = function() {
      ...
      launch = 'Iniciado em ' + month + '/' + day + ' ' + hour + ':' + min + ' ' + meridian + '. ';
      totals = '' + t.songs + ' Acumulou :+1: ' + t.woots + ' :-1: ' + t.mehs + ' e :heart: ' + t.curates + '.';
      msg = launch + totals;
      return API.sendChat(msg);
    };
  ...
  unhookCommand = (function(_super) {
    ...
    unhookCommand.prototype.init = function() {
      this.command = '!unhook events all(NÃO USAR)';
      this.parseType = 'exact';
  ...
  dieCommand = (function(_super) {
    ...
    dieCommand.prototype.functionality = function() {
      API.sendChat('Desativando eventos...');
      undoHooks();
      API.sendChat('Deletando historico...');
      data.implode();
      return API.sendChat('Considere-me desligado');
    };
  ...
  overplayedCommand = (function(_super) {
    ...
    overplayedCommand.prototype.functionality = function() {
      return API.sendCha("View the list of songs we consider overplayed and suggest additions at http://den.johnback.us/overplayed_tracks");
    };
        ...
        for (id in _ref29) {
          ...
          if (u.getUser().username === givenName) {
            ...
            if (disconnectInstances.length > 0) {
              resp = u.getUser().username + ' desconectou ' + disconnectInstances.length.toString() + '.';
              ...
              resp += 'Seu desconect mais recente foi as ' + dcTimeStr + ' (' + dcSongsAgo + ' musicas atras). ';
  ...
  voteRatioCommand = (function(_super) {
    ...
    voteRatioCommand.prototype.functionality = function() {
      ...
      if (msg.length > 12) {
        ...
        return API.sendChat("Faça isso direito...");
      }
    };
  ...
  announceCurate = function(obj) {
    return API.sendCha("/em: " + obj.user.username + " loves this song!");
  };
